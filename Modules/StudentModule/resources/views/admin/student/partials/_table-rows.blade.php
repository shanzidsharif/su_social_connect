@forelse ($items as $key => $item)
    <tr>
        ...
        <td class="text-nowrap align-middle">{{ $item->student->department ?? '' }}</td>
        <td class="text-nowrap align-middle">{{ $item->student->subject ?? '' }}</td>
        <td class="text-nowrap align-middle">{{ $item->student->student_id ?? '' }}</td>
            ...
            <div class="form-group">
                <label class="custom-switch form-switch mb-0 pt-2">
                    ...
                    <input type="checkbox" {{ $item['is_active'] == 1 ? 'checked' : '' }}
                        onchange="ajax_post('{{ route('admin.students.status-update', $item['id']) }}','','')"
            ...
            <div class="form-group">
                <label class="custom-switch form-switch mb-0 pt-2">
                    ...
                    <input type="checkbox" {{ $item['is_verified'] == 1 ? 'checked' : '' }}
                        onchange="ajax_post('{{ route('admin.students.verification-update', $item['id']) }}','','')"
                        class="custom-switch-input">
                    <span class="custom-switch-indicator"></span>
                    <span class="custom-switch-description">On</span>
                </label>
        ...
        <td class="text-center align-middle">
            <div class="g-2">
                <a class="btn text-primary btn-sm" data-bs-toggle="tooltip" data-bs-original-title="Edit"
                    href="{{ route('admin.students.edit', $item['id']) }}"><span
