                        ...
                        <div class="d-flex order-lg-2">
                            ...
                            <div class="dropdown d-flex profile-1">
                                <a href="javascript:void(0)" data-bs-toggle="dropdown"
                                    ...
                                    <img @if (!empty(auth()->user()['profile_image'])) src="{{ asset('storage/users/profile_images') }}/{{ auth()->user()['profile_image'] }}"
                                        @else
                                            src="{{ asset('assets/default-user-icon.jpg') }}" @endif
                                ...
                                <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                    <div class="drop-heading">
                                        <div class="text-center">
                                            ...
                                            <small
                                                class="text-muted text-capitalize">{{ auth()->user()->user_type }}</small>
                                        </div>
                                    </div>
                                    <div class="dropdown-divider m-0"></div>
                                    <a class="dropdown-item" href="{{ route('teacher.profile.get') }}">
                                        <i class="dropdown-icon fe fe-user"></i> Profile
                                    </a>
                                    <a class="dropdown-item" href="javascript:void(0)" id="logout-button">
                                        <i class="dropdown-icon fe fe-alert-circle"></i>Logout
                                    </a>
                                    <form action="{{ route('teacher.auth.logout') }}" method="post" id="logout-form"
                                        class="hide">
                                        @csrf
                                    </form>
                                </div>
